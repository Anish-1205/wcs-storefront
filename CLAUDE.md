...
A Next.js 15 (App Router) storefront + admin panel for a saree wholesale/retail business. Public catalog with WhatsApp-based lead capture (no e-commerce checkout — inquiries and WhatsApp are the conversion path), backed by Supabase (Postgres + Auth), Cloudinary for images, Upstash Redis for rate limiting, Resend for email notifications, and Sentry for error tracking.

**Two data sources, on purpose.** As of Sept 2026 the customer-facing storefront (`src/app/(public)/**`) is **file-driven** — it reads from `src/data/products.ts` / `src/data/collections.ts` with media in `public/media/<slug>/`, and adds a client-side enquiry cart that hands off to WhatsApp. The **admin panel and import pipeline (`src/app/admin/**`, `src/lib/queries.ts`, all Supabase/Cloudinary) are unchanged** and still run against Postgres. See [docs/storefront-catalogue.md](docs/storefront-catalogue.md) for the storefront (product data, media pipeline, cart, WhatsApp handoff, env vars, the "nothing is invented" claims rule). The Supabase sections below still describe the admin half. A one-way mirror (`syncFileProducts`, `src/app/admin/sync-actions.ts`, "Sync storefront products" button on `/admin/products`) copies the file catalog into `products`/`product_variants`/`variant_images`/`collections` so every live product is visible/manageable in admin — mirrored rows are tagged `products.source = 'file_sync'` (migration 013) and never overwrite an admin-authored product at the same slug; saved photos and primary-image choices on published mirrored rows now flow to the live catalog through `storefront-media` overrides; saved names, descriptions, highlights and base prices also overlay the file entries. Re-sync preserves existing product and collection edits. Public page content and homepage placement are editable at `/admin/pages` after migrations 017 and 018 (018 adds unpublished drafts and published-version history behind the plain-language editor); see [docs/website-pages.md](docs/website-pages.md) for setup, rendering, cache invalidation and remaining file-catalog boundaries.
...
**Product media import pipeline** (`/admin/import`, `supabase/migrations/009_import_pipeline.sql`): bulk desktop/mobile photo & video ingestion — direct-to-Cloudinary signed uploads (`/api/import/sign`, `/api/import/complete`), heuristic grouping into proposed products (`src/lib/import/grouping.ts`), and fail-closed collection assignment (`src/lib/import/collection-classification.ts`) that only ever reaches `confirmed` via a deterministic source or explicit admin action — AI can only produce a `suggested` state requiring confirmation. AI suggestions go through a small provider abstraction (`src/lib/ai/`, Anthropic by default, always safely degrading to a no-op provider). **Naming/tagging consistency is enforced in code, not just prompted**: `src/lib/ai/style-guide.ts` is the single source of truth for the catalogue convention (`[Colour] [Fabric/Weave] Saree with [Key Detail]`, plus highlight and colour-label rules) and is used both as prompt text and as deterministic post-processing (`enforceNameStyle` / `resolveProductName` / `normalizeHighlights`) applied to every AI response and to the AI-free fallbacks — so a product reads the same whichever batch or session made it. The real taxonomy goes into the prompt as a closed list (`existingCategories`, `existingCollectionNames`) and `category_slug` is re-filtered against it defensively, same as collection ids. Existing products can be brought up to the standard via **/admin/products → "Re-run AI naming & tagging"** (`src/app/admin/enrichment-actions.ts` preview→apply, `src/lib/enrichment/reprocess.ts`), which only ever writes name/fabric/highlights/category/added collection tags — never images, variants, prices, status or the slug. Within a confirmed group, AI can also cluster photos into colour variants and flag a best-looking default (`suggestColorVariants` → `ai_color_variants` draft, migration 014) — same fail-closed shape: `requestGroupColorVariantSuggestions` only ever writes a draft, `applyGroupColorVariants` (`src/lib/import/color-variants.ts`) is the explicit admin confirm that sets `import_assets.variant_group`, and only then does `createProductFromGroup` split into one `product_variants` row per colourway instead of a single "Default" one. Imported products are always `draft` with `review_status = 'pending_review'` and are blocked from publishing (in both `saveProduct` and `updateProductStatus`) until `approveImportedProductReview` is called; pre-existing products are unaffected (`review_status = 'not_required'`). See [docs/import-pipeline.md](docs/import-pipeline.md) for the full design and known gaps (video assets aren't attached to the product yet; visual-similarity *grouping* of raw uploads into products, as opposed to colour-variant clustering within an already-grouped product, still isn't implemented).

## Testing

- Unit tests (Vitest) live in `src/__tests__/` and cover validation schemas, API routes (inquiries, whatsapp, upload), webhook security, security policies, the WhatsApp ingestion flow (product creation + photo-append, mocked Supabase/Meta/Cloudinary), and the import pipeline's fail-closed logic (grouping heuristics, collection classification, colour-variant assignment) — run with `npm test`.
