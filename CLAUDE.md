...
**Two data sources, on purpose.** As of Sept 2026 the customer-facing storefront (`src/app/(public)/**`) is **file-driven** — it reads from `src/data/products.ts` / `src/data/collections.ts` with media in `public/media/<slug>/`, and adds a client-side enquiry cart that hands off to WhatsApp. The **admin panel and import pipeline (`src/app/admin/**`, `src/lib/queries.ts`, all Supabase/Cloudinary) are unchanged** and still run against Postgres. See [docs/storefront-catalogue.md](docs/storefront-catalogue.md) for the storefront (product data, media pipeline, cart, WhatsApp handoff, env vars, the "nothing is invented" claims rule). The Supabase sections below still describe the admin half. A one-way mirror (`syncFileProducts`, `src/app/admin/sync-actions.ts`, "Sync storefront products" button on `/admin/products`) copies the file catalog into `products`/`product_variants`/`variant_images`/`collections` so every live product is visible/manageable in admin — mirrored rows are tagged `products.source = 'file_sync'` (migration 013) and never overwrite an admin-authored product at the same slug; saved photos and primary-image choices on published mirrored rows now flow to the live catalog through `storefront-media` overrides; saved names, descriptions, highlights and base prices also overlay the file entries. Re-sync preserves existing product and collection edits. Public page content and homepage placement are editable at `/admin/pages` after migration 017; see [docs/website-pages.md](docs/website-pages.md) for setup, rendering, cache invalidation and remaining file-catalog boundaries.
