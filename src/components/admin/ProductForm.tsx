...
export function ProductForm({ categories, collections, initial }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {/* Two columns on wide screens: the form keeps its readable measure and
          the preview takes the space that was empty to the right of it. The
          preview sticks, so the saree stays visible while the admin works
          down the form instead of having to scroll back to the photo grids.
          Below xl there is no room beside the form, so it sits on top. */}
      <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_22rem]">
        <aside className="order-first xl:order-none xl:col-start-2 xl:row-start-1 xl:sticky xl:top-4 xl:self-start">
          <ProductPreview
            name={name}
